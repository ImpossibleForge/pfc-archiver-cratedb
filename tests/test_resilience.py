"""
Resilience & stress tests for pfc-archiver-cratedb
====================================================
Mirrors the S-series (failure scenarios) and CrateDB-specific bonus tests
from the pfc-archiver-java test report.

All tests run without a live CrateDB — psycopg2 and subprocess are mocked.
...
class TestDBDownAtStartup(unittest.TestCase):
    """S1: CrateDB is unreachable when archive_cycle runs."""
    ...
    def test_connection_error_cycle_fails_gracefully(self, mock_connect):
        ...
        mock_connect.side_effect = Exception("Connection refused: localhost:5432")
    ...
    def test_connection_error_logs_cycle_failed(self, mock_connect):
        ...
        cfg = _make_cfg()
        import logging
        with self.assertLogs("pfc-archiver", level="ERROR") as cm:
            archive_cycle(cfg, "/usr/bin/pfc_jsonl", dry_run=False)
        self.assertTrue(
    ...
    def test_db_error_during_export_skips_partition_continues_others(
        ...
        cfg = _make_cfg()
        with patch("pfc_archiver.upload_archive", return_value=True), \
             patch("pfc_archiver.verify_archive", return_value=True), \
             patch("pfc_archiver.write_run_log") as mock_log:
            archive_cycle(cfg, "/usr/bin/pfc_jsonl", dry_run=False)
...
class TestBinaryMissing(unittest.TestCase):
    ...
    def test_compress_failure_skips_partition_daemon_continues(self, mock_connect):
        ...
        with patch("pfc_archiver.get_partition_ranges", return_value=[p1, p2]), \
             patch("pfc_archiver.subprocess.run") as mock_run:
...
class TestTableNotFound(unittest.TestCase):
    ...
    def test_nonexistent_table_cycle_fails_gracefully(self, mock_connect):
        ...
        mock_cur.execute.side_effect = Exception(
            'relation "doc"."nonexistent_table" does not exist'
        )
        mock_conn.cursor.return_value = mock_cur
        mock_connect.return_value = mock_conn
...
class TestUnwritableOutputDir(unittest.TestCase):
    ...
    def test_local_copy_permission_error_is_raised(self):
        ...
        with patch("pfc_archiver.shutil.copy2",
...
class TestDeleteOrdering(unittest.TestCase):
    ...
    @patch("pfc_archiver.export_partition_to_pfc")
    @patch("pfc_archiver.get_partition_ranges")
    def test_delete_runs_when_verify_disabled(
        self, mock_partitions, mock_export, mock_upload,
        mock_verify, mock_delete, mock_log
    ):
...
class TestMultipleCyclesNoLeak(unittest.TestCase):
    ...
    def test_connection_closed_after_get_partition_ranges(self, mock_connect):
        """DB connection must be closed after get_partition_ranges."""
        now    = _now()
...
# ===========================================================================
# CC1 — Large batch streaming (CrateDB)
# ===========================================================================
...
class TestLargeBatchStreaming(unittest.TestCase):
    ...
    def test_50k_rows_processed_in_batches(self, mock_connect, mock_run):
        """50,000 rows should be fetched in multiple batches, not all at once."""
        TOTAL_ROWS  = 50_000
        BATCH_SIZE  = 10_000
...
# ===========================================================================
# CC2 — Schema-qualified queries verified at query level
# ===========================================================================

class TestSchemaQualifiedQueries(unittest.TestCase):
    """CrateDB queries must always use schema-qualified table names."""

    @patch("pfc_archiver._connect")
    def test_export_uses_schema_qualified_table(self, mock_connect):
        """export_partition_to_pfc must use "schema"."table" in its query."""
        mock_conn = MagicMock()
        mock_cur  = MagicMock()
        mock_cur.description = [("timestamp",), ("level",)]
        mock_cur.fetchmany.return_value = []
        ...
        db_cfg  = {
            ...
            "table": "events", "ts_column": "timestamp",
        }
        from_ts = datetime(2026, 3, 1, tzinfo=timezone.utc)
        to_ts   = datetime(2026, 3, 2, tzinfo=timezone.utc)
    ...
    @patch("pfc_archiver._connect")
    def test_default_schema_doc_used_when_not_specified(self, mock_connect):
        """When schema not in db_cfg, defaults to 'doc'."""
        ...
        # No schema key in db_cfg — must default to "doc"
...
class TestColumnTypeSerialization(unittest.TestCase):
    ...
    def test_various_types_dont_crash(self, mock_connect, mock_run):
        ...
        db_cfg  = {
            "host": "localhost", "schema": "doc",
            "table": "sensors", "ts_column": "timestamp",
        }
        from_ts = datetime(2026, 3, 1, tzinfo=timezone.utc)
        to_ts   = datetime(2026, 3, 2, tzinfo=timezone.utc)
    ...
    def test_bytes_serialized_as_hex(self, mock_connect, mock_run):
        """bytes values must be serialized as hex strings."""
        mock_conn = MagicMock()
        mock_cur  = MagicMock()
...
# ===========================================================================
# CC5 — Multiple partitions in one cycle (5 days of data)
# ===========================================================================
